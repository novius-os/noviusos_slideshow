<?php

// Generated on 23/01/2013 11:25:53

// 35 out of 35 messages are translated (100%).
// 145 out of 145 words are translated (100%).

return array(
    #: views/admin/_form_content.view.php:43
    #: views/admin/_form_content.view.php:109
    'Image' => 'Image',

    #: views/admin/_form_content.view.php:80
    'Add a slide' => 'Ajouter une diapo',

    #: views/admin/_form_content.view.php:113
    'Are you sure you want to delete this slide?' => 'Êtes-vous sûr(e) de vouloir supprimer cette diapo&nbsp;?',

    #: views/admin/preview.php:13
    'Slideshow ‘{{title}}’' => 'Diaporama «&nbsp;{{title}}&nbsp;»',

    #: views/admin/preview.php:14
    '(format: {{format}})' => '(format&nbsp;: {{format}})',

    #: views/admin/warning_not_published.view.php:15
    'Not published' => 'Non publié',

    #: views/admin/warning_not_published.view.php:16
    'To publish this slideshow, add it to a page, a blog post or any other text editor.' => 'Pour publier ce diaporama, ajoutez-le à une page, un billet de blog ou toutautre éditeur de texte.',

    #: views/admin/warning_not_published.view.php:19
    '<p>Most Novius OS text editors (also called WYSIWYG editors) feature this button {{preview}} in the toolbar.</p><p>Click it to access the list of applications you can enhance your text with.</p>' => '<p>La plupart des éditeurs de texte de Novius OS (aussi appelés éditeursWYSIWYG) ont ce bouton {{preview}} dans la barre d’outils.</p><p>Cliquezpour accéder à la liste des applications permettant d’enrichir voscontenus.</p>',

    #: views/admin/warning_not_published.view.php:20
    'Applications' => 'Applications',

    #: views/admin/_form_image.php:12
    'Properties' => 'Propriétés',

    #: views/admin/_form_image.php:19
    'Image:' => 'Image&nbsp;:',

    #: views/admin/_form_image.php:37
    'Slide name:' => 'Nom de la diapo&nbsp;:',

    #: views/admin/_form_image.php:39
    'Slide name' => 'Nom de la diapo',

    #: views/admin/_form_image.php:44
    'Description:' => 'Description&nbsp;:',

    #: views/admin/_form_image.php:53
    'Links to:' => 'Lien vers&nbsp;:',

    #: views/admin/_form_image.php:58
    'Add a link' => 'Ajouter un lien',

    #: views/admin/_form_image.php:85
    'Remove the link' => 'Retirer le lien',

    #: config/controller/admin/slideshow.config.php:17
    #: config/common/slideshow.config.php:66
    'Add a slideshow' => 'Ajouter un diaporama',

    #: config/controller/admin/slideshow.config.php:44
    #: config/common/slideshow.config.php:9
    'Title' => 'Titre',

    #: config/controller/admin/slideshow.config.php:58
    'Save' => 'Enregistrer',

    #: config/controller/admin/appdesk.config.php:21
    'slideshow' => 'diaporama',

    #: config/controller/admin/appdesk.config.php:22
    'slideshows' => 'diaporamas',

    #: config/controller/admin/appdesk.config.php:23
    'Showing {{x}} slideshows out of {{y}}' => '{{x}} diaporamas sur {{y}} affichés',

    #: config/controller/admin/appdesk.config.php:24
    'Showing 1 slideshow' => '1 diaporama affiché',

    #: config/controller/admin/appdesk.config.php:25
    'No slideshows' => 'Pas de diaporama',

    #: config/controller/admin/appdesk.config.php:26
    'Show all slideshows' => 'Afficher tous les diaporamas',

    #: config/common/slideshow.config.php:24
    'Slides' => 'Diapos',

    #. Crud
    #: config/common/slideshow.config.php:48
    'Added! This is a nice new slideshow you have here.' => 'C’est ajouté. En voilà un beau diaporama.',

    #: config/common/slideshow.config.php:49
    'The slideshow has been deleted.' => 'Ce diaporama a été supprimé.',

    #. General errors
    #: config/common/slideshow.config.php:52
    'This slideshow doesn’t exist any more. It has been deleted.' => 'Ce diaporama n’existe plus. Il a été supprimé.',

    #: config/common/slideshow.config.php:53
    'We cannot find this slideshow.' => 'Nous n’arrivons pas à trouver ce diaporama. Êtes-vous sûr(e) qu’ilexiste&nbsp;?',

    #. Deletion popup
    #: config/common/slideshow.config.php:56
    'Deleting the slideshow ‘{{title}}’' => 'Supprimer le diaporama «&nbsp;{{title}}&nbsp;»',

    #. Delete action's labels
    #: config/common/slideshow.config.php:59
    'Yes, delete this slideshow' => 'Oui, supprimer ce diaporama',

    #: config/common/slideshow.config.php:61
    '1 slideshow' => '1 diaporama',

    #: config/common/slideshow.config.php:62
    '{{count}} slideshows' => '{{count}} diaporamas',

);
