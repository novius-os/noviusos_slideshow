<?php

// Generated on 09/04/2013 16:13:10

// 44 out of 44 messages are translated (100%).
// 165 out of 165 words are translated (100%).

return array(
    #: config/controller/admin/slideshow.config.php:19
    #: config/common/slideshow.config.php:74
    'Add a slideshow' => 'スライドショーを追加',

    #: config/controller/admin/slideshow.config.php:46
    #: config/common/slideshow.config.php:9
    'Title' => 'タイトル',

    #. Note to translator: This is a submit button
    #: config/controller/admin/slideshow.config.php:61
    'Save' => '保存',

    #: config/controller/admin/appdesk.config.php:21
    'slideshow' => 'スライドショー',

    #: config/controller/admin/appdesk.config.php:22
    'slideshows' => 'スライドショー',

    #: config/controller/admin/appdesk.config.php:23
    'Showing {{x}} slideshows out of {{y}}' => '{{y}}件のスライドショーのうち{{x}}件を表示しています',

    #: config/controller/admin/appdesk.config.php:24
    'Showing 1 slideshow' => '1件のスライドショーを表示しています',

    #: config/controller/admin/appdesk.config.php:25
    'No slideshows' => 'スライドショーはありません',

    #. Note to translator: This is the action that clears the 'Search' field
    #: config/controller/admin/appdesk.config.php:27
    'Show all slideshows' => 'スライドショーを全て表示する',

    #: config/slideshow.config.php:19
    'Big' => '大',

    #: config/slideshow.config.php:25
    'Small' => '小',

    #: config/common/slideshow.config.php:31
    'Slides' => 'スライド',

    #. Crud
    #: config/common/slideshow.config.php:56
    'Added! This is a nice new slideshow you have here.' => '新しいスライドショーを追加しました。',

    #: config/common/slideshow.config.php:57
    'The slideshow has been deleted.' => 'スライドショーが削除され亜mした。',

    #. General errors
    #: config/common/slideshow.config.php:60
    'This slideshow doesn’t exist any more. It has been deleted.' => 'スライドショーが存在しません。削除されました。',

    #: config/common/slideshow.config.php:61
    'We cannot find this slideshow.' => 'このスライドショーは見つかりませんでした。',

    #. Deletion popup
    #: config/common/slideshow.config.php:64
    'Deleting the slideshow ‘{{title}}’' => 'スライドショー ‘{{title}}’ を削除します',

    #. Delete action's labels
    #: config/common/slideshow.config.php:67
    'Yes, delete this slideshow' => 'はい、このスライドショーを削除します',

    #: config/common/slideshow.config.php:69
    '1 slideshow' => '1個のスライドショー',

    #: config/common/slideshow.config.php:70
    '{{count}} slideshows' => '{{count}}個のスライドショー',

    #: views/admin/_form_image.view.php:15
    'Properties' => 'プロパティ',

    #: views/admin/_form_image.view.php:22
    'Image:' => '画像:',

    #: views/admin/_form_image.view.php:40
    'Slide name:' => 'スライド名:',

    #: views/admin/_form_image.view.php:42
    'Slide name' => 'スライド名',

    #: views/admin/_form_image.view.php:47
    'Description:' => '説明:',

    #: views/admin/_form_image.view.php:56
    'Links to:' => 'リンク先:',

    #: views/admin/_form_image.view.php:61
    'Add a link' => 'リンクを追加',

    #: views/admin/_form_image.view.php:88
    'Remove the link' => 'リンクを取り除く',

    #: views/admin/_form_content.view.php:46
    #: views/admin/_form_content.view.php:113
    'Image' => '画像',

    #: views/admin/_form_content.view.php:84
    'Add a slide' => 'スライドを追加',

    #: views/admin/_form_content.view.php:90
    'Delete' => '削除',

    #: views/admin/_form_content.view.php:117
    'Pick an image' => '画像を選択',

    #: views/admin/_form_content.view.php:118
    'Pick another image' => '別の画像を選択',

    #: views/admin/_form_content.view.php:119
    'No image' => '画像がありません',

    #: views/admin/_form_content.view.php:120
    'This extension is not allowed.' => 'この拡張子の画像は許可されていません',

    #: views/admin/_form_content.view.php:124
    'Are you sure you want to delete this slide?' => 'このスライドを削除してもよろしいですか？',

    #: views/admin/warning_not_published.view.php:18
    'Not published' => '未公開',

    #: views/admin/warning_not_published.view.php:19
    'To publish this slideshow, add it to a page, a blog post or any other text editor.' => 'このスライドショーを公開するには、ページ、ブログ投稿、あるいはその他のテキストエディタに追加してください。',

    #: views/admin/warning_not_published.view.php:22
    '<p>Most Novius OS text editors (also called WYSIWYG editors) feature this button {{preview}} in the toolbar.</p><p>Click it to access the list of applications you can enhance your text with.</p>' => '<p>ほとんどの Novius OS テキストエディタ (WYSIWYG エディタとも呼びます) のツールバーに、ボタン {{preview}} があります。</p><p>このボタンをクリックすると、アプリケーションの一覧にアクセスし、アプリケーションを追加できます。</p>',

    #: views/admin/warning_not_published.view.php:23
    'Applications' => 'アプリケーション',

    #: views/admin/enhancer/preview.view.php:16
    'Slideshow ‘{{title}}’' => 'スライドショー ‘{{title}}’',

    #: views/admin/enhancer/preview.view.php:17
    '(format: {{format}})' => '(フォーマット: {{format}})',

    #: views/admin/enhancer/popup.view.php:15
    'Select a slideshow:' => 'スライドショーを選択:',

    #: views/admin/enhancer/popup.view.php:30
    'Format:' => 'フォーマット:',

);
