<?php

// Generated on 03/12/2013 16:22:23

// 39 out of 40 messages are translated (97%).
// 217 out of 223 words are translated (97%).

return array(
    #: views/admin/_form_content.view.php:36
    'Add a slide' => 'Добавить слайд',

    #: views/admin/_form_content.view.php:42
    'Delete' => 'Удалить',

    #: views/admin/_form_content.view.php:59
    'Are you sure you want to delete this slide?' => 'Вы уверены, что хотите удалить этот слайд?',

    #: views/admin/enhancer/preview.view.php:16
    'Slideshow ‘{{title}}’' => 'Слайд-шоу «{{title}}»',

    #: views/admin/enhancer/preview.view.php:17
    '(format: {{format}})' => '(формат: {{format}})',

    #: views/admin/enhancer/blank_slate.view.php:16
    'How frustrating, you have no slideshow to insert. But let’s not worry, shall we? Here is how it works:' => 'К сожалению, у вас нет слайд-шоу для вставки. Но не будем расстраиваться, вот что нужно сделать:',

    #: views/admin/enhancer/blank_slate.view.php:19
    '<ul><li><a>Add your first slideshow</a> (a new tab will open).</li>
<li>Once you’re done (it won’t take long), come back to this tab to insert your shiny new slideshow.</li></ul>' => '<ul><li><a>Добавить ваше первое слайд-шоу</a> (откроется новая вкладка).</li>
<li>Как только вы закончите (это займет немного времени), возвращайтесь на эту вкладку, чтобы вставить новехонькое слайд-шоу.</li></ul>',

    #: views/admin/enhancer/blank_slate.view.php:27
    'No slideshows are available in {{context}}. Go ahead, <a>add your first slideshow in this language</a>.' => 'Слайд-шоу недоступны в {{context}}. Давайте <a>добавим первое слайд-шоу на этом языке</a>.',

    #: views/admin/enhancer/blank_slate.view.php:29
    'No slideshows are available in {{context}}. Go ahead, <a>add your first slideshow in this context</a>.' => 'Слайд-шоу недоступны в {{context}}. Давайте <a>добавим первое слайд-шоу в этот контекст</a>.',

    #: views/admin/warning_not_published.view.php:18
    'Not published' => 'Не опубликовано',

    #: views/admin/warning_not_published.view.php:19
    'To publish this slideshow, add it to a page, a blog post or any other text editor.' => 'Чтобы опубликовать это слайд-шоу, добавьте его на страницу, в пост блога или в любом другом текстовом редакторе.',

    #: views/admin/warning_not_published.view.php:22
    '<p>Most Novius OS text editors (also called WYSIWYG editors) feature this button {{preview}} in the toolbar.</p><p>Click it to access the list of applications you can enhance your text with.</p>' => '<p>Большинство текстовых редакторов Novius OS (также называемых WYSIWYG-редакторами) содержат кнопку {{preview}} на панели.</p><p>Нажмите ее для доступа к списку приложений, с помощью которых вы сможете оформить ваш текст.</p>',

    #: views/admin/warning_not_published.view.php:23
    'Applications' => 'Приложения',

    #: views/admin/renderer/page_selector.view.php:6
    'Add a link' => 'Добавить ссылку',

    #: views/admin/renderer/page_selector.view.php:10
    'Remove the link' => 'Удалить ссылку',

    #: config/slideshow.config.php:17
    'Big Flexslider v1.8' => 'Большой Flexslider v1.8',

    #: config/slideshow.config.php:37
    'Small Flexslider v1.8' => 'Маленький Flexslider v1.8',

    #: config/controller/admin/slideshow.config.php:19
    #: config/common/slideshow.config.php:79
    'Add a slideshow' => 'Добавить слайд-шоу',

    #: config/controller/admin/slideshow.config.php:45
    #: config/common/slideshow.config.php:9
    'Title' => 'Заголовок',

    #: config/controller/admin/slideshow.config.php:68
    'Image:' => 'Изображение:',

    #: config/controller/admin/slideshow.config.php:76
    'Image' => 'Изображение',

    #: config/controller/admin/slideshow.config.php:92
    'Slide name:' => 'Наименование слайда:',

    #: config/controller/admin/slideshow.config.php:95
    'Description:' => 'Описание:',

    #: config/controller/admin/slideshow.config.php:105
    'Links to:' => 'Ссылки на:',

    #: config/controller/admin/slideshow.config.php:119
    'Properties' => 'Свойства',

    #: config/controller/admin/enhancer.config.php:22
    'Select a slideshow:' => 'Выберите слайд-шоу:',

    #: config/controller/admin/enhancer.config.php:28
    'Format:' => 'Формат:',

    #: config/controller/admin/appdesk.config.php:21
    'slideshow' => 'слайд-шоу',

    #: config/controller/admin/appdesk.config.php:22
    'slideshows' => 'слайд-шоу',

    #: config/controller/admin/appdesk.config.php:24
    #: config/common/slideshow.config.php:73
    '1 slideshow' => array(
        0 => '1 слайд-шоу',
        1 => '{{count}} слайд-шоу',
        2 => '',
    ),

    #: config/controller/admin/appdesk.config.php:28
    'Showing 1 slideshow out of {{y}}' => array(
        0 => '',
        1 => '',
        2 => 'Показаны {{x}} слайд-шоу из {{y}}',
    ),

    #: config/controller/admin/appdesk.config.php:31
    'No slideshows' => 'Нет слайд-шоу',

    #. Note to translator: This is the action that clears the 'Search' field
    #: config/controller/admin/appdesk.config.php:33
    'Show all slideshows' => 'Показать все слайд-шоу',

    #: config/common/slideshow.config.php:31
    'Slides' => 'Слайды',

    #. Crud
    #: config/common/slideshow.config.php:56
    'Added! This is a nice new slideshow you have here.' => 'Добавлено. Вот новое слайд-шоу.',

    #: config/common/slideshow.config.php:57
    'The slideshow has been deleted.' => 'Слайд-шоу было удалено.',

    #. General errors
    #: config/common/slideshow.config.php:60
    'This slideshow doesn’t exist any more. It has been deleted.' => 'Это слайд-шоу больше не существует.  Оно было удалено.',

    #: config/common/slideshow.config.php:61
    'We cannot find this slideshow.' => 'Невозможно найти это слайд-шоу.',

    #. Deletion popup
    #: config/common/slideshow.config.php:64
    'Deleting the slideshow ‘{{title}}’' => 'Удаление слайд-шоу «{{title}}»',

    #: config/common/slideshow.config.php:68
    'Yes, delete this slideshow' => array(
        0 => 'Да, я хочу удалить это слайд-шоу.',
        1 => '',
        2 => '',
    ),

);
